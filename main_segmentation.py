"""
Real-time instance segmentation system with multi-camera support.
"""
import cv2
import time
import signal
...
class InstanceSegmentationSystem:
    """Main instance segmentation system."""
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info("Initializing instance segmentation detector...")
    ...
    def _save_screenshots(self, frames: Dict[int, np.ndarray]) -> None:
        ...
        for camera_id, frame in frames.items():
            processed_frame = self.process_frame(frame, camera_id)
            filename = f"segmentation_screenshot_camera_{camera_id}_{timestamp}.jpg"
            cv2.imwrite(filename, processed_frame)
            logger.info(f"Segmentation screenshot saved: {filename}")
    ...
    def stop(self) -> None:
        ...
        logger.info("Instance segmentation system stopped")

def main():
    """Main function."""
    print("🎯 Real-time Instance Segmentation System v1.0")
    print("===============================================")
    print("Features:")
    print("- Multi-camera support (RTSP + local cameras)")
    print("- Real-time YOLOv11 instance segmentation")
    ...
    system = InstanceSegmentationSystem()
